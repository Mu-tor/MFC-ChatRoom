...
void test2(void *p){
	...
	while(1)
	{
		...
		memset(name1,0,64);
		recv(cDlg->clsoc, recvbuff, 2, 0);
		if (recvbuff[0] == 't' && recvbuff[1] == 'o') {
			recv(cDlg->clsoc, recname1, 64, 0);
			msg=recname1;//接收用户
			if (msg == cDlg->name) {//是否发送给当前用户,否则丢弃(给我的)
				recv(cDlg->clsoc, name1, 64, 0);
				msg=name1;//发送用户
				if (msg == cDlg->recname) {//是否为当前对话框用户发送的消息(谁发的)
					...
					recv(cDlg->clsoc, recvbuff, 1024, 0);//接受后丢弃
