...
void ForgetDlg::OnOK() 
{
	...
	if(name==""||password1==""||password2==""||question==""||encrypted==""||code2==""){
		...
		password2=="";
	}else if(code2[0]!=code[0]||code2[1]!=code[1]||code2[2]!=code[2]||code2[3]!=code[3]){
		MessageBox("验证码都能输错？");
		code2=="";
	}else {
		...
		if(rval>0){
			if(recvbuff[0]=='y'&&recvbuff[1]=='e'&&recvbuff[2]=='s'){
				...
				MessageBox("系统错误!");
				this->EndDialog(-1);//关闭对话框
			}else if(recvbuff[0]=='a'&&recvbuff[1]=='g'&&recvbuff[2]=='a'&&recvbuff[3]=='i'&&recvbuff[4]=='n'){
				MessageBox("请重试!");
				this->EndDialog(-1);//关闭对话框
			}else if(recvbuff[0]=='-'&&recvbuff[1]=='1'){
				MessageBox("系统错误,代码-1!");
				this->EndDialog(-1);//关闭对话框
			}else if(recvbuff[0]=='-'&&recvbuff[1]=='2'){
				MessageBox("系统错误,代码-2!");
				this->EndDialog(-1);//关闭对话框
			}else if(recvbuff[0]=='-'&&recvbuff[1]=='5'){
				MessageBox("系统错误,代码-5!");
				this->EndDialog(-1);//关闭对话框
			}else if(recvbuff[0]=='-'&&recvbuff[1]=='6'){
				MessageBox("系统错误,代码-6!");
				this->EndDialog(-1);//关闭对话框
			}
		}
	}
