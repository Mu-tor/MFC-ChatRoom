...
BOOL CServerDlg::OnInitDialog()
{
	...
	//初始化用户列表
	m_list.InsertColumn(0, "用户", 0, 120);
...
void test(void *p)
{
	...
	do{
		...
		if(y>0){
			if(recvbuff[0]=='q'&&recvbuff[1]=='u'&&recvbuff[2]=='i'&&recvbuff[3]=='t'){
				//找到clientsoc并删除
				offline(client->name);
				for(int i=0; i<MaxListen; ++i){
				...
				sendUser();
				break;
			}else if(recvbuff[0]=='l'&&recvbuff[1]=='o'&&recvbuff[2]=='g'&&recvbuff[3]=='i'&&recvbuff[4]=='n'){
				//用户发来登陆请求
			...
			else if (recvbuff[0] == 's' && recvbuff[1] == 'i' && recvbuff[2] == 'g' && recvbuff[3] == 'n' && recvbuff[4] == 'i' && recvbuff[5] == 'n') {
				//用户发来注册请求
			...
			else if (recvbuff[0] == 't' && recvbuff[1] == 'o') {
				for (int i = 0; i < MaxListen; i++) {
...
int uplist(void *p) {
	...
	if (!db.OpenDataBase("student.mdb"))
	{
		...
		if (!db.Query((char*)(LPCSTR)str))
			...
			sDlg->m_list.SetItemText(i, 1, line[0] == 49 ? "在线" : "离线");//添加在线状态
			db.Next();
			i++;
		} while (!db.IsEof());
		db.Close();
